...
def display_haishoku(art_image):
    ...
    for art_image in dirs:
        if os.path.isfile(file+art_image):
            if art_image == '.DS_Store': 
                continue
            # for art_image in img_path:
            hai = Haishoku.loadHaishoku(file+art_image)
            palette = Haishoku.getPalette(file+art_image)
...
if __name__ == "__main__":
    ...
    art_image = load_artworks(data)

    load_thumbnail(art_image)

    display_haishoku(art_image)

    palette = display_haishoku(art_image)

    load_color_palette(palette)
